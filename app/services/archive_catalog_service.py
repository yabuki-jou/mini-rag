...
def _field_value(field: ArchiveFieldValue):
    """读取字段实际使用的值列。"""
...
def _field_map(document_id: UUID, session: Session) -> dict[ArchiveFieldName, ArchiveFieldValue]:
    ...
    return {field.field_name: field for field in _read_field_values(document_id, session)}


def _enum_value(field: ArchiveFieldValue | None, enum_type):
    """把数据库文本字段安全转换为固定字典值。"""
    if field is None or _field_value(field) is None:
        return None
    try:
        return enum_type(str(_field_value(field)))
    except ValueError:
        return None
...
def _build_archive_summary(
    *,
    ...
    title = _field_value(fields.get(ArchiveFieldName.TITLE)) if fields.get(ArchiveFieldName.TITLE) else None
    organization = _field_value(fields.get(ArchiveFieldName.AUTHORING_ORGANIZATION)) if fields.get(ArchiveFieldName.AUTHORING_ORGANIZATION) else None
    document_date = _field_value(fields.get(ArchiveFieldName.DOCUMENT_DATE)) if fields.get(ArchiveFieldName.DOCUMENT_DATE) else None
...
def _blocked_document_ids(project_id: UUID, session: Session) -> set[UUID]:
    """读取当前项目存在可见性阻断操作的文档集合。"""
    rows = session.exec(
...
def list_formal_archives(
    *,
    ...
    for document, archive_document in rows:
        ...
        current_date = _field_value(fields.get(ArchiveFieldName.DOCUMENT_DATE)) if fields.get(ArchiveFieldName.DOCUMENT_DATE) else None
        current_org = _field_value(fields.get(ArchiveFieldName.AUTHORING_ORGANIZATION)) if fields.get(ArchiveFieldName.AUTHORING_ORGANIZATION) else None
...
def read_formal_archive(*, document: Document, session: Session) -> ArchiveDetailRead:
    ...
    if document.id in _blocked_document_ids(document.project_id, session):
        raise AppError(404, "ARCHIVE_NOT_FORMAL", "该档案当前不在正式范围。")
    fields = _field_map(document.id, session)
...
def list_audit_logs(
    *,
    ...
    rows = list(
        session.exec(
            statement.order_by(ArchiveAuditLog.created_at.desc(), ArchiveAuditLog.id.desc())
        ).all()
    )
    total = len(rows)
    start = (page - 1) * page_size
