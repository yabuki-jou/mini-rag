...
分数；C.3-A 已通过开发环境隐藏诊断接口观测完整 Top-20，确认 `GROUNDED-01/05/07` 未进入候选池；C.3-B 已在同一 Top-20 与本地 Reranker 下复验 `values` 表示，63/63 个 Chunk 获得上下文但结果仍为有据 `7/8`、无据拒答 `0/2`、隔离 `2/2`，不存在可冻结的独立重排阈值。Chunk 调整、查询表达、换模型或修改评测集均需另行重新授权；DeepSeek 问答质量、P13 真实跨存储故障恢复和完整 Vue 工作台
