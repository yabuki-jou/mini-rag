...
开发过程中出现错误时，可以自主排查和修复。

### 哪些错误需要完整排查并记录

只有当错误属于**项目本身的实际缺陷**时，才进入完整调试流程并制作错误修复文档。
满足以下任意一种情况即可：
- 项目功能行为与需求或预期不一致
- 生产代码存在逻辑错误
- API、数据结构、数据库或模块之间的契约不一致
- 可能造成数据错误、资源泄漏、误删除或状态错误
- 自动化测试暴露了真实的生产代码缺陷
- 问题可以稳定复现，并且需要修改项目代码才能解决

处理过程：
**错误现象 → 排查 → 根因 → 修改方案 → 修复 → 验证**

根因确认后再说明为什么采用当前修改方案。
修复完成后，将问题记录到对应功能的错误修复文档中。

---

### 哪些错误不要记录

以下属于开发过程中的临时错误，可以自行修正，不需要进入错误修复文档：

- 命令参数写错
- Shell / PowerShell 语法或引号错误
- 临时路径或文件名写错
- 使用了错误的 Python / 虚拟环境
- Agent ID、工具参数或工具调用错误
- 搜索命令范围错误
- Patch 上下文不匹配
- 临时诊断脚本自身的问题
- 测试夹具或测试代码本身的明显错误
- TDD 中预期出现的 RED 测试
- 沙箱、权限或本地开发环境限制
- 日志读取、会话输出、临时探针等辅助操作失败
- 不涉及项目代码修改的一次性操作错误

这些问题最多在最终汇报中简单提及，不要单独制作错误记录。
...
> **这个问题是否说明项目代码本身存在缺陷？**
如果答案是否定的，就不要记录。
如果只是“Codex 在操作过程中做错了一步，然后自己纠正”，不属于项目缺陷。
...
- 简单、明确的局部修改，不需要立即运行完整测试。
- 调试过程中，优先运行能够复现当前问题的最小测试。
- 一个功能或一组相关修改完成后，再运行相关测试。
- 整个任务完成后，进行一次完整验证。
- 如果相关测试已经通过，且代码没有发生影响该测试的修改，不要无意义地重复运行。
- 不要为了“保险”反复执行相同测试。

测试的目的应是验证具体假设和修改结果，而不是机械地增加测试次数。

# 多 Agent 开发工作流

## 主 Agent

GPT-6 Sol中 作为主 Agent。

主 Agent 负责：需求分析、文档审查、架构决策、实现方案规划、任务拆分和最终实现审查。在进入代码实现阶段之前，主 Agent 必须先产出清晰、明确、可执行的实现计划（Implementation Plan）。

## 实现 Agent

当实现任务已经足够明确、边界清晰且适合独立执行时，应优先将任务委派给 GPT-6 Luna高 子 Agent。Luna 负责代码实现、测试编写、常规 Bug 修复、机械性重构、Lint/格式化处理和重复性批量修改，并严格按照既定实现计划执行。

Luna 不得自行修改整体架构、既定需求、任务范围、对外公开 API 或核心数据库结构。如发现必须进行上述变更，应停止扩大修改，并交回 GPT-6 Sol中 主 Agent 重新分析、决策和更新实现计划。

## 实现审查

代码实现完成后，由 GPT-6 Sol中 主 Agent 审查 Git Diff、实现计划完成度、需求符合性、测试完整性、API 与数据模型一致性、范围扩张、遗漏、偏离和不必要修改。发现问题时，主 Agent 生成范围清晰的修正任务，再委派 GPT-6 Luna高 执行。
