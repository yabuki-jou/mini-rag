...
用户已明确授权将正式档案检索交给本地 Reranker 的 Chroma 候选池固定扩大为 Top-20；公开接口仍最多返回 10 条。C.3-A 已增加仅开发环境可用的完整 Top-20 脱敏诊断，C.3-B 已获授权并完成 `values` 表示 + 现有本地 Reranker 的单变量复验；这些步骤均不改变公开检索行为。除已完成的候选池、诊断观测和 `values` 复验外，不得自动执行 Chunk 调整、查询表达、换模型或评测集修改；C.3-C 仍需独立产品/范围授权。
...
## 注意事项
- 每次代码改动后都必须创建一个对应的commit并添加相应的合理注释，方便追踪
