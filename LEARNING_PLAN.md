...
- 2026-09-13 制度 Agent 当前评测已迁入 `evals/policy_agent/`：6 条固定样例使用真实 DeepSeek 和注入的虚构制度结果完成评测，契约与人工语义评分共 `12/12` 为 `1.0`，Step 6 完整性检查通过；相关测试 `12 passed`，后端全量回归 `438 passed, 2 skipped, 127 warnings`，`compileall` 与 `git diff --check` 通过。该结果不证明真实 Chroma 检索；准备阶段实际发现当前 768 维 Embedding 与既有 512 维制度 Collection 不兼容，真实制度检索仍需隔离重建或迁移后复验。已删除请假领域的旧 Pixie 运行器、数据集和追踪。
