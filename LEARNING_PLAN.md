...
- 2026-09-23 FR-042 P153548 术语补充召回的真实回答/引用对照已完成：在独立 Chroma/evidence_values 链路上执行五组 Top-8、每组 3 次，共 15/15 次 DeepSeek 调用；C 组（`IBRD IDA` 重排）冻结标注引用 3/3，A/B/D/E 为 0/3 并保留非冻结来源人工复核，不据此确定正式融合规则。真实 Chroma、PostgreSQL、文件和 Checkpoint 均完成隔离清理；定向测试 `26 passed`，后端全量 `522 passed, 2 skipped`，`compileall` 通过。当前仍未修改正式 `app/`，下一步先明确引用验收口径并扩充反向正例/其他问题评估，再决定是否另立正式 Implementation Plan。
- 2026-09-25 已确认 DEC-024：冻结集继续按指定目标证据评分，扩展来源集将人工核实的其他真实来源支持单独评分，不替代或合并冻结结果。扩展样例方案已写入 `docs/review/FR-042-项目档案助手/P153548扩展来源双口径评测样例方案.md`；白俄罗斯官方公告仍待制作带来源哈希的 TXT 快照与逐段标注，尚未执行新检索或模型评估。
