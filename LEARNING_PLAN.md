...
- 2026-09-14 V1 候选基线主审确认 services 分包、旧导入清理和现有行为回归通过；同时发现企业评测清单的项目级元数据仍引用四个已删除 service 路径。按 TDD 补充项目级来源存在性检查，RED 为 `1 failed, 9 passed`，更新四个路径后 GREEN 为 `10 passed`；完整回归 `487 passed, 2 skipped, 130 warnings`，`compileall app tests scripts evals` 与差异检查通过。该修复尚未提交，本地 `v1.0.0` 仍指向修复前的 `1f2d092`。
