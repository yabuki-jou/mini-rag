...
def test_ask_question_rejects_without_calling_model(
    ...
    monkeypatch.setattr(
        "app.services.rag.chat.read_recent_messages",
        Mock(return_value=[]),
    )
    monkeypatch.setattr(
        "app.services.rag.chat.retrieve_chunks",
        Mock(return_value=[]),
    )
    monkeypatch.setattr(
        "app.services.rag.chat.get_chat_model",
        model_factory,
    )
    monkeypatch.setattr(
        "app.services.rag.chat.save_chat_exchange",
        save_exchange,
    )
