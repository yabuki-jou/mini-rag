...
from app.services.rag.embeddings import EmbeddedChunk
...
def test_ensure_chunk_collection_creates_cosine_collection(
    ...
    monkeypatch.setattr(chroma, "get_chroma_client", lambda: client)
...
def test_chroma_client_uses_project_namespace(
    ...
    monkeypatch.setattr(chroma.chromadb, "HttpClient", http_client)

    assert chroma.get_chroma_client() is client
