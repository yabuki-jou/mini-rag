...
| 文档状态 | FR-019～FR-041 数据库切片已实现；FR-042 MVP 数据库设计、API 契约与 Implementation Plan 已确认，P00～P07 已通过，0011 会话范围迁移及随机隔离 PostgreSQL schema 的项目删除服务联动门已完成 |
| 更新日期 | 2026-09-16 |
...
- `FOREIGN KEY (project_id, kb_id) REFERENCES projects(id, kb_id) MATCH SIMPLE ON DELETE CASCADE`，约束名
  ...
  `MATCH FULL`。
- 新增 `INDEX ix_agent_sessions_project_id (project_id)`，用于项目删除前读取其档案会话线程。
  继续保留 `user_id`、`kb_id` 与唯一 `thread_id` 索引；本期没有会话列表，不增加排序复合索引。
