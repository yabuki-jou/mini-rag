...
当前正式数据模型服务智慧档案 V1、FR-039 和 FR-042。旧通用 RAG、普通 Chat 和制度 Agent 的表、历史数据与迁移不删除、不改写，但已不再有公开运行入口；`POLICY` 仅作为历史会话行兼容值，档案入口必须拒绝。

## 1. 文档信息

| 项目 | 内容 |
|---|---|
| 对应基线 | `docs/design/需求说明.md` 的 FR-019～FR-042、BR-008～BR-039、NFR-011～NFR-024；`docs/design/技术架构.md` 与 DEC-019/DEC-020 |
| 文档状态 | FR-019～FR-041 数据库切片已实现；FR-042 MVP 的 P00～P08 已通过确定性实现与回归，0011 会话范围迁移及随机隔离 PostgreSQL schema 的项目删除服务联动门已完成；P08 复用既有项目索引且没有新增迁移 |
| 更新日期 | 2026-09-18 |
...
业务 API 的实现状态以 `docs/design/接口设计.md` 和 `docs/implementation/智慧档案V1实施计划.md` 为准。
...
5. LangGraph Checkpoint 使用独立于 PostgreSQL 的 SQLite 文件；项目档案助手复用同一受保护文件，
   以全局唯一 `thread_id` 隔离。它不保存归档业务事实，也不与本设计中的表建立外键。
...
| Checkpoint SQLite | 制度与档案 Agent 的可序列化执行状态 | 项目、档案和审计数据 |

## 3. 与现有表的关系

### 3.1 保留的旧表

`users`、`knowledge_bases`、`documents`、聊天表、Agent 会话表和工具日志表继续
存在。`users` 在 AV1-A01 中受控增加登录标识与密码哈希，新增 `auth_sessions` 保存可撤销的
Refresh Token 会话；旧用户仍保留但可没有凭据。FR-042 只扩展现有 `agent_sessions` 的会话类型
和项目绑定，不新增轮次、历史引用或会话删除状态表。
...
erDiagram
    ...
    KNOWLEDGE_BASES ||--o{ AGENT_SESSIONS : scopes
...
- 删除空项目时保留其 `kb_id` 指向的 `knowledge_bases` 记录。该知识库可能仍承载旧
  历史 RAG/Agent 练习文档；项目删除只清理项目及其级联项目数据，不承担历史数据清理。
...
| `date_value` | DATE NULL | 仅 `DOCUMENT_DATE` 使用；合同资料表示合同签订日期，其他资料表示文档自身日期 |
...
- `DOCUMENT_DATE` 只能使用 `date_value`；合同资料的该值表示合同签订日期，其他资料表示
  文档自身日期。关键词只能使用非空 JSON 字符串数组；其他字段只能
...
### 6.11 `agent_sessions` 与 `agent_tool_call_logs`（FR-042 最小扩展）

FR-042 复用现有 Agent 会话与工具日志表。PostgreSQL 只保存会话归属、项目绑定和脱敏审计；
消息正文继续保存在 Checkpoint SQLite。本期不创建 `agent_turns`、`agent_turn_citations` 或会话
删除状态列。
...
|---|---|---|
| `id` | UUID PK | 既有会话 ID |
| `user_id` | UUID NOT NULL FK → `users.id` | 会话所有者；客户端不能提交 |
| `kb_id` | UUID NOT NULL FK → `knowledge_bases.id` | 服务端绑定的检索范围 |
| `project_id` | UUID NULL | 档案会话所属项目；制度会话必须为空 |
| `agent_type` | VARCHAR(16) NOT NULL DEFAULT `POLICY` | `POLICY` 或 `ARCHIVE`；存量行回填为 `POLICY` |
| `thread_id` | VARCHAR(100) NOT NULL UNIQUE | 共用 Checkpoint 文件中的全局唯一线程标识 |
| `created_at` / `updated_at` | TIMESTAMP WITHOUT TIME ZONE NOT NULL | 既有应用层 UTC 时间字段；本期不改时区存储契约 |

约束与索引：

- `CHECK (agent_type IN ('POLICY', 'ARCHIVE'))`，约束名
  `ck_agent_sessions_agent_type`；物理列使用字符串加 CHECK，避免只为两个稳定值新增 PostgreSQL
  Enum，并保持 SQLite 测试结构一致。
- `CHECK ((agent_type = 'POLICY' AND project_id IS NULL) OR
  (agent_type = 'ARCHIVE' AND project_id IS NOT NULL))`，约束名
  `ck_agent_sessions_type_project`。
- `FOREIGN KEY (project_id, kb_id) REFERENCES projects(id, kb_id) MATCH SIMPLE ON DELETE CASCADE`，约束名
  `fk_agent_sessions_project_kb`。PostgreSQL 的 `MATCH SIMPLE` 语义允许制度会话的 `project_id`
  为空；档案会话则由该复合外键保证项目与知识库一致。必须在迁移 DDL 中
  显式写出 `MATCH SIMPLE`，不得依赖数据库默认值，防止后续误改为会拒绝存量制度会话的
  `MATCH FULL`。
- 新增 `INDEX ix_agent_sessions_project_id (project_id)`，用于项目删除前读取其档案会话线程，也用于
  P08 最近会话恢复先收窄当前项目范围。继续保留 `user_id`、`kb_id` 与唯一 `thread_id` 索引；
  P08 只在单项目的小规模会话集合内按 `updated_at`、`created_at` 和 `id` 排序，不提供通用列表，
  因此不新增排序复合索引或 Alembic 迁移。
- 不对 `project_id` 建唯一约束；一个项目可以显式创建多个会话。会话创建、读取和执行仍由
  Service 校验 `agent_type + user_id + project_id + kb_id`，数据库复合外键不能替代项目所有者校验。

#### `agent_tool_call_logs`

字段、`AgentToolCallStatus` 和 `UNIQUE(agent_session_id, tool_call_id)` 保持不变。唯一结构调整是把
既有 `agent_session_id → agent_sessions.id` 外键改为 `ON DELETE CASCADE`，使项目删除级联删除
档案会话时不会留下工具日志。工具日志不增加 `project_id` 或 `agent_type` 冗余列；读取时必须先
通过已验证的 AgentSession 限定所有者和会话类型。

#### 删除与 Checkpoint 顺序

项目删除仍要求项目没有任何文档。删除服务先在 PostgreSQL 事务中锁定 Project、读取
`agent_type='ARCHIVE' AND project_id=:project_id` 的会话 ID/线程 ID，但在外部 Checkpoint 清理前
不提交项目删除；随后幂等删除这些 Checkpoint 线程，最后删除 Project。PostgreSQL 通过
`fk_agent_sessions_project_kb ON DELETE CASCADE` 删除档案会话，再通过工具日志外键级联删除日志。

Project 行锁从读取档案会话线程一直持有到 PostgreSQL 提交，因此会跨越 Checkpoint
文件删除。在此窗口内，同项目的档案会话创建与文档上传必须阻塞；这是本地单实例
MVP 为了防止清理集合在删除中途变化而接受的有界代价，不得把同一锁扩大到模型调用或
普通消息执行。

项目绑定的 `knowledge_bases` 记录和 `project_id IS NULL` 的制度会话继续保留。SQLite 测试若未启用
外键级联，Service 必须按“工具日志 → 档案会话 → Project”显式删除，以保持与 PostgreSQL 相同的
可观察结果。该顺序不提供跨 PostgreSQL/SQLite 原子事务，也不覆盖消息与项目删除并发；任一层
失败都不得返回删除成功。
...
| 动作 | PostgreSQL 事实 | Checkpoint/文件/Chroma 动作 | 关键约束 |
|---|---|---|---|
...
| 删除文档 | 创建可见性阻断删除操作，最终物理删除 | 删除向量、原文件和快照 | 失败保留未完成删除操作，计数不减 |
| 创建档案助手会话 | 写入 `ARCHIVE + project_id + kb_id` 的 `agent_sessions` | 不写 Checkpoint，不调用模型或 Chroma | 项目归属由 Service 验证；类型/项目和项目/知识库约束防止误绑定 |
| 发送档案助手消息 | 更新会话时间，以唯一调用 ID 合并脱敏工具审计 | Checkpoint 保存完整轮次；工具只读 Chroma | 不新增轮次/引用表；任一最终失败返回 503，历史忽略不完整轮次 |
| 恢复最近档案助手会话 | 只读当前用户、项目、知识库和 ARCHIVE 类型下按稳定顺序排序的首条会话 | 不直接读取 Checkpoint | 无会话返回正常空结果；随后由既有历史和工具日志接口分别读取持久内容 |
| 删除空项目 | 删除 Project，级联删除当前项目的档案会话和工具审计 | 先幂等删除这些会话的 Checkpoint 线程 | 任一层失败不返回成功；保留 KnowledgeBase 及其制度会话；MVP 不验收并发删除 |
...
FR-042 预计以 `0011_archive_agent_scope` 作为下一个前向迁移，该 revision id
长度为 24，低于 Alembic 默认 `alembic_version.version_num VARCHAR(32)` 上限。
`down_revision` 指向真实 revision id `0010_account_auth`（对应迁移文件
`0010_account_password_authentication.py`），不得把文件名主干当作 revision id。该迁移
尚未创建，必须在本设计、接口设计和实施计划均确认后才进入 TDD 实现。
...
7. `0011` 先为 `agent_sessions` 新增 `agent_type VARCHAR(16) NOT NULL DEFAULT 'POLICY'`，
   使存量会话自然回填为制度会话，再增加 `ck_agent_sessions_agent_type`。
8. 新增可空 `project_id`、`ix_agent_sessions_project_id`、
   `ck_agent_sessions_type_project` 和显式 `MATCH SIMPLE` 的复合外键
   `fk_agent_sessions_project_kb`；不为存量制度会话伪造项目绑定。
9. 替换 `agent_tool_call_logs.agent_session_id` 既有匿名外键。PostgreSQL 迁移必须先通过
   Inspector 按“受限列 `agent_session_id` + 引用表 `agent_sessions`”精确找到旧外键；
   当前预期的 PostgreSQL 默认名为 `agent_tool_call_logs_agent_session_id_fkey`，但不得
   猜测一个自定义旧名。删除旧外键后，以具名约束
   `fk_agent_tool_logs_session_cascade` 重建对 `agent_sessions.id` 的引用并增加
   `ON DELETE CASCADE`。SQLite 测试迁移使用 Alembic batch alter 和 naming convention
   为反射到的匿名外键生成可删除名，再重建表约束。
10. `0011_archive_agent_scope.py` 的 `COLUMN_COMMENTS["agent_sessions"]` 必须提供
    `agent_type` 与 `project_id` 注释；同一 TDD 切片必须修改
    `tests/models/test_schema_comment_contract.py`，加载并合并 `0011` 注释映射。只写
    PostgreSQL COMMENT 而不更新该既有契约测试，不算完成。该迁移不创建 AgentTurn、
    引用、会话删除状态或第二份 Checkpoint 文件。

回滚只允许在本地开发环境执行。`0011` 降级前必须确认不存在 `ARCHIVE` 会话；
否则停止降级，不得静默丢失项目绑定。降级恢复工具日志旧外键后，再删除
FR-042 的外键、检查约束、索引和两列。已有归档或档案会话数据时不得通过
删除迁移或 `alembic stamp` 掩盖状态，应使用新的前向迁移处理。
...
- `0011` 前向迁移后，所有存量 AgentSession 均为 `POLICY + project_id NULL`，原有制度
  Agent 可观察行为不变。
- `POLICY + project_id 非空`、`ARCHIVE + project_id 为空`和非法 `agent_type` 均被数据库拒绝。
- 档案会话的 `(project_id, kb_id)` 不匹配时插入失败；同一项目可创建多个档案会话，
  但 `thread_id` 仍全局唯一。
- 删除空项目后，该项目的档案会话、工具日志和 Checkpoint 线程不可再读取；
  项目绑定的 KnowledgeBase 及其制度会话仍存在。
- PostgreSQL 外键级联与 SQLite 测试环境的显式“工具日志 → 档案会话 → Project”
  删除路径得到相同的可观察结果。
- Schema 中不存在 FR-042 专用 AgentTurn、历史引用或会话删除状态表/列。

FR-042 数据库行为按两层最小证据验收：

1. 确定性 TDD 使用 SQLite，但涉及复合外键或级联的测试连接必须显式执行
   `PRAGMA foreign_keys=ON`。在该环境验证项目/知识库错绑被拒绝、项目删除级联和
   Service 显式删除路径的可观察结果；不得使用未开启外键的
   `tests/test_migration_service.py` 现有连接冒充这两项证据。
2. 目标 PostgreSQL 集成门验证真实 migration head 为 `0011_archive_agent_scope`、
   旧匿名外键已替换、新约束名与 `MATCH SIMPLE/ON DELETE CASCADE` 实际存在，并对一组
   错绑拒绝与级联删除做最小实证。SQLite 通过不能替代这个 PostgreSQL 门。
3. 注释契约测试必须纳入 `0011` 的两个新列注释，并继续要求合并后键集与
   SQLModel 实际列集完全一致。对既有测试的这项修改必须明确写入 Implementation Plan。

FR-019～FR-041 的 API 契约已在 `docs/design/接口设计.md` 固定：版本值使用请求体、
删除恢复复用 `DELETE`、字段响应和稳定错误码已有明确约定。FR-042 API 已完成主审修正并由用户确认，
且遵守本设计的服务端项目绑定和会话类型隔离。`archive_final_chunks` 的实际字段长度、
向量维度、索引参数和相关性阈值必须按 `docs/review/验证冻结清单.md` 的门槛 2
验证后冻结。

当前已创建 SQLModel 与 `0005`～`0011` Alembic 迁移。项目/清单、上传、状态机、Final Collection、
目录、检索、问答和删除 Router/Service 已完成对应实现切片；P13 真实恢复、D6-B 固定质量门、
企业扩展集和相邻 Vue 分阶段真实代理链路已有独立证据。FR-042 的 `AgentSession` 类型/项目范围、
`0011_archive_agent_scope` 和真实 PostgreSQL 约束门已通过；P03 会话创建服务已使用该范围写库。
数据库结构本身仍不能替代真实模型、Chroma 和前端联调证据；
独立空库迁移自动化、云端容量和生产恢复尚未验证，详见验收报告。
