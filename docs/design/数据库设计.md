...
| 项目 | 内容 |
|---|---|
| 对应基线 | `docs/design/需求说明.md` V1.2 的 FR-019～FR-041、BR-008～BR-029、NFR-011～NFR-020；`docs/design/技术架构.md` 已确认架构基线 |
| 文档状态 | AV1-P03 模型/迁移和 A01、P06～P14 对应切片已实现；真实 PostgreSQL/Chroma 纵向证据、正式 Collection 切换、P13 恢复、D6-B 固定质量门和相邻 Vue 分阶段真实代理链路已记录 |
| 更新日期 | 2026-09-11 |
...
API 契约已在 `docs/design/接口设计.md` 固定：版本值使用请求体、删除恢复复用 `DELETE`、
字段响应和稳定错误码已有明确约定。`archive_final_chunks` 的实际字段长度、向量维度、
索引参数和相关性阈值必须按 `docs/review/验证冻结清单.md` 的门槛 2
验证后冻结。
...
企业扩展集和相邻 Vue 分阶段真实代理链路已有独立证据。数据库结构本身仍不能替代这些证据；
