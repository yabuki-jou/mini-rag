...
| 文档状态 | AV1-P03 模型/迁移和 A01、P06～P13 对应切片已实现；真实 PostgreSQL/Chroma 纵向证据、P11 单文档 P95 与 P13 真实跨存储恢复已记录。P14 固定集尚不能冻结检索阈值，真实 DeepSeek 和端到端验收仍待完成 |
| 更新日期 | 2026-08-28 |
...
目录、检索、问答和删除 Router/Service 已完成对应实现切片；固定质量门槛、真实 DeepSeek
和完整端到端验收仍不能因数据库结构存在而视为已完成；P13 真实跨存储故障恢复已有单次本地验收证据。
