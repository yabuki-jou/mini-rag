...
`PARSED`、`PARSE_FAILED`、`ParsedSnapshot` 表和项目解析/重试 API 已实现；P09～P14 的确认、索引、
目录、检索、问答、删除和本地验收均已接入后续流程，最终证据边界见验收报告。
...
验证代码位于 `tests/services/archive/test_parser.py`，运行时动态生成虚构 TXT/MD、DOCX、
