...
| 项目 | 内容 |
|---|---|
| 对应需求 | `docs/design/需求说明.md` 的 FR-019～FR-041、BR-008～BR-029、NFR-011～NFR-020 |
| 文档状态 | 架构、数据库与 API 设计基线已确认；AV1-A01 与 P01～P13 实现切片已完成，包含 AI 建议、确认—INDEX/取消确认、清单关联/目录/审计、正式检索、证据问答和物理删除。P14 的固定集质量、真实 DeepSeek、真实跨存储恢复和完整 Vue 联调仍未验收 |
| 更新日期 | 2026-08-28 |
| 当前业务方向 | 智慧档案与企业文档智能 |
| 架构风格 | 同步 FastAPI 模块化单体 |

本文件描述智慧档案 V1 的目标架构。当前代码已实现项目、清单、上传解析、AI/手工草稿、
确认—INDEX/取消确认、正式目录、清单关联、审计、正式检索、证据问答与物理删除的服务和路由切片；
它不表示固定问题集质量、真实模型质量或完整端到端验收已经通过。
...
`docs/design/Chroma迁移决策.md`。`vector_service`、检索、删除、健康检查和 Compose 已完成
...
索引的 `evidence_values` 实验模式会额外将已人工确认、且具有当前快照 FieldEvidence 的字段值
注入其定位匹配的 Final Chunk 向量输入；它不改变 Chroma 保存的原文、对外摘录或定位。若字段标记为
无原文证据、证据快照过期、定位不包含或锚点不匹配，则该字段不参与向量上下文。该模式的固定问题集
真实质量尚未完成验收，默认表示与距离阈值仍未冻结。
...
数据库与 API 契约已在 `docs/design/数据库设计.md` 与 `docs/design/接口设计.md` 固定。实施计划已
...
- **[DONE – FOUNDATION TO P13]** AV1-P03 的迁移/模型/授权基础、P04～P13 的业务 Router、
  ...
  P11 单文档基线也已有本地证据。
- **[TODO – P14 QUALITY]** 纯稠密 BGE/Chroma 固定集未能冻结单一阈值；已确认仅增加本地 Reranker，
  对授权和范围校验后的 Chroma Top-20 候选重排，并以固定集标定独立重排阈值，才可继续 P11/P12 的真实质量验收。
- **[TODO – P14 INTEGRATION]** 执行真实 P13 跨存储故障恢复运行器，并完成 FR-034～FR-041 Vue 联调、
  文档收口和最终回归。

## 19. 当前下一步

当前下一步为 AV1-P14：先处理 P11 固定集无法冻结阈值的产品/技术边界，再执行真实 P13 恢复、
P12 真实问答质量和 Vue 工作台后续接口联调；所有结论必须记录实际运行证据。
