...
当前正式架构只服务智慧档案 V1、FR-039 档案问答和 FR-042 项目档案助手。通用知识库、普通 Chat 和制度 Agent 已下线；历史表、数据、迁移、Checkpoint、Collection 和文件保留用于兼容与追溯，不构成当前运行入口。
...
真实链路的通过范围以 `docs/review/验收报告.md` 为准。

FR-042 使用“PostgreSQL 会话归属 + SQLite Checkpoint 消息状态 + 脱敏工具审计”的机制，使用独立的档案 Graph 与工具集合，并按全局唯一
`thread_id` 复用同一受保护 Checkpoint 文件。SQLite 仍不是档案字段、确认状态、正式可见性或
项目绑定的业务事实来源。

向量存储已由 Milvus 改为 Chroma 单机服务模式，详见
`docs/design/Chroma迁移决策.md`。`infrastructure/chroma.py`、档案检索/删除、健康检查和 Compose 已完成
...
6. 档案字段、字段证据、清单关联、Agent 会话归属和工具审计保存为 PostgreSQL 业务事实；
   项目档案助手使用同一受保护 SQLite Checkpoint 文件，以全局唯一 `thread_id`
...
8. 项目档案助手不复用历史制度 Agent 的 Prompt、Graph State 或工具语义；历史 `POLICY`
   会话只保留兼容行，档案入口必须拒绝它们，不引入运行时多 Agent 编排。
...
| 历史 `app/services/rag/parser.py` | 旧 PDF/DOCX/TXT/MD 文本提取 | 已下线；档案解析位于 `archive/parser.py` |
| 历史 `app/services/rag/chunks.py` | 旧分块与稳定 Chunk ID | 已下线；正式档案使用 `archive/final_chunks.py` |
| `app/services/infrastructure/ai_models.py` | 本地 BGE Embedding 与 DeepSeek 客户端 | 当前共享基础设施 |
| `app/services/infrastructure/chroma.py` | Chroma 客户端 | 档案 Final Collection 位于 `archive/final_chunks.py` |
| 历史 `app/services/rag/retrieval.py`、`prompting.py` | 旧知识库检索和聊天 Prompt | 已下线；FR-039 使用档案检索与问题服务 |
| `app/services/infrastructure/ai_models.py` | DeepSeek 与 Embedding 客户端 | 分别服务于建议与 Grounded 问答 |
| `app/agents/archive/` | 项目档案助手 Checkpoint、会话消息与工具审计 | 当前唯一 Agent 运行时；历史制度 Agent 不再接入 |
...
删除协调仍放在 `app/services/agent/` 下的档案专用模块。除非实现中出现真实重复，不提前抽象
通用基类，也不恢复历史制度 Agent 的对外入口。
...
FR-042 使用受保护的 Agent Checkpoint SQLite 文件，以全局唯一 `thread_id` 保存并隔离可恢复
会话状态。该文件可以保留历史 `POLICY` 线程，但当前没有制度 Agent 运行入口。P14 的 Vue 工作台只能经由
...
- 项目档案助手入口：先解析 `ProjectContext`，再按 `session_id + user_id + project_id + kb_id +
  agent_type=ARCHIVE` 读取 ACTIVE 会话；任一不匹配统一按资源不可用处理。历史 `POLICY` 行没有
  公开运行入口，也不能读取档案 Checkpoint 或工具日志。
...
消息和来源转换使用档案专用投影；已删除的制度消息投影和 `search_company_policy` 提取逻辑
不得恢复为兼容层。
...
| 历史制度 Agent | 仅保留历史资料与兼容数据 | 不属于当前运行能力，不读取或执行项目档案会话 |
...
| AC-12 | §6.2 与 §10.7.4 的 ARCHIVE 五要素校验、线程隔离和历史 POLICY 会话拒绝 |
...
| 历史 POLICY 线程误入档案助手 | 会话类型、项目绑定、独立 Graph 和档案入口五要素校验 | 同一 Checkpoint 文件仍可保留历史线程，需持续验证线程隔离 |
