...
证据问答和物理删除路由；FR-042 已实现独立 DTO、会话创建、双向类型隔离、两个内部只读工具、Archive Graph、共享 Checkpoint Runtime、
消息执行、历史与工具日志四端点闭环。本地验收状态和证据边界以
`docs/implementation/验收报告.md` 为准。
...
| FR-042 | 服务端项目绑定、会话类型双向隔离、输入规范化/额外字段拒绝、连续两轮、目录/证据工具选择、脱敏引用、无据拒答、完整轮次历史、工具审计去重、稳定 503 与项目删除联动 |
...
其他用户或制度 Agent。项目本身不存在或不属于当前用户时，仍分别使用既有
...
会话类型固定为 `ARCHIVE`，同时避免客户端把该响应误用为既有制度 Agent 会话 DTO。
...
| AC-04、AC-08、AC-12、AC-14 | `extra="forbid"`、消息规范化、会话五要素查找、POLICY/ARCHIVE 双向隔离与工具参数白名单 |
