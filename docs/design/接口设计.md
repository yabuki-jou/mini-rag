...
| 文档状态 | API 设计基线已确认；AV1-A01、FR-030～FR-041 的后端实现切片和确定性测试已完成。P14 固定集证明当前纯稠密方案不能冻结阈值；P13 真实恢复已完成，真实 DeepSeek 和完整 Vue 端到端验收仍待完成 |
| 更新日期 | 2026-08-28 |
...
| 智慧档案 V1 | `/projects` | FR-030～FR-041 的路由切片已实现；P13 跨存储故障恢复已验收，固定检索质量、真实 DeepSeek 和端到端验收仍待 P14 | 工程项目归档、清单、正式目录、检索、问答与审计 |
...
引用；`top_k` 默认 `5`、最大 `10`。阶段 C 固定以 Chroma Top-20 作为候选池，完成范围复核后才
本地重排。兼容字段 `score` 仍为 `1 - Chroma cosine distance`；`reranker_score` 是最终排序和独立
重排阈值使用的分数。该阈值尚未通过固定集标定。没有依据返回 `200` 与空 `items`。不得引用待确认、
...
`docs/design/智慧档案V1解析器设计.md`；BGE/Chroma 行为、`min_relevance_score` 与跨存储恢复
仍按 `docs/review/验证冻结清单.md` 分阶段验证后冻结。
