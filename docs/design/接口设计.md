...
| 文档状态 | AV1-A01、FR-030～FR-041 API 契约已实现并通过既定验收；FR-042 MVP API 契约和 Implementation Plan 已确认，P00～P07 已通过，四个项目档案助手端点与项目删除联动已闭环 |
| 更新日期 | 2026-09-16 |
...
四个端点都必须携带 Bearer Access Token。服务先从路径项目构造已验证的
...
本期不新增会话列表、会话详情、重命名、用户删除、历史引用恢复、流式响应、
