...
> 清单项 CRUD/派生状态 API、P05 项目内上传、P06 正式解析/快照/重试、P07 手工草稿/字段检查、P08 AI 建议/安全 regenerate、P09 确认/INDEX/取消确认、P10 清单关联/目录/审计、P11 正式检索、P12 证据问答和 P13 物理删除切片；真实确认—INDEX、Chroma/BGE canary、取消确认清理和单文档 P95 基线已通过。固定检索质量、真实 DeepSeek、P13 真实跨存储恢复和完整端到端验收仍待 P14。它不复用本 Agent 作为归档状态或档案问答的实现依据。

> **档案 V1 当前状态（2026-08-27）：** P09 已完成真实确认—INDEX 路由链路和取消确认清理；P10～P13
> 已实现清单关联/目录/审计、正式检索、证据问答和物理删除切片。固定检索质量、真实 DeepSeek、
> P13 真实跨存储恢复和完整端到端验收仍待 P14。本 Agent 仍只负责制度检索，不承担档案业务状态。
